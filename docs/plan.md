...
- [ ] Decide whether to backport to the Tauri app (frozen, but still shipped until cutover) the terminal-link fixes the native client got in P2.3: Tauri runs a Ctrl+clicked executable or script without asking and touches any UNC host a link or session folder names
