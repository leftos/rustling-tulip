...
<!-- plan-doc-hygiene: 2026-09-26 f66a2a7 -->
...
- [x] The `Shutdown` handler comment in `crates/daemon/src/server.rs` (~2163, "Pre-Phase-C the children die anyway…") predates tracers: sessions now survive a daemon exit and are reattached or listed as abandoned
...
- [x] Fix the 8 Information-level PSScriptAnalyzer findings in `rt.ps1` (PSAvoidUsingPositionalParameters on `Join-Path`)
