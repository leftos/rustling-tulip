...
- [ ] `.\rt.ps1 build` fails in a shell with no `vswhere.exe` on PATH: `Get-VsWherePath` finds it by absolute path, but `VsDevCmd.bat` then calls it by bare name (`rt.ps1` ~300-308). Put its directory on PATH for the `VsDevCmd` call
