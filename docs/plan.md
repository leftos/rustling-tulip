...
- [ ] **UX audit triage** — see `docs/ux-audit.md` for the 4 remaining open code findings and the hand-test checklist. Testid gaps and the first wave of failing-test→fix→green specs are all resolved (see `docs/plans/e2e-test-coverage-strategy.md`). Remaining open items: empty-state when sidebar has workspaces but only orphan sessions (`Sidebar.tsx:550`); `TabWindow.focusedPaneId` independent from main window focus; `SessionWindow` ignoring broadcast `Repos`/`Workspaces`; pop-out window has no close confirmation (`App.tsx:354`). Ongoing — stop when the polish-to-feature ratio gets uncomfortable.
...
- [ ] Does `claude --add-dir` propagate hooks/settings from each root, or only the cwd? (Test empirically + check Context7 docs — still unverified)
- [x] When a workspace member is *itself* nested inside another, how should worktree paths be laid out? **Resolved by implementation:** `git::workspace_worktree_paths` places all members under a shared `<worktrees-root>/<sanitized-anchor>/wt.<branch-slug>/` root, preserving inter-repo relative offsets. Cross-drive members fall back to leaf-name placement and lose relativity for that member only.
- [x] Should workspace session spawn refuse if any member repo has uncommitted changes? **Resolved by implementation:** `checkout_in_place` in `crates/daemon/src/git.rs` hard-errors on a dirty working tree — the session refuses to spawn. "Warn but allow" was not implemented; the current behavior is a hard refusal.
