...
- **Duplicate on a fresh branch** — duplicating a worktree session spawns the clone on a daemon-picked `wt/` name under `RefuseLeftover` instead of replaying the source's branch (which either collided with the running source's worktree or could attach a leftover). In-place duplicates still replay the branch. See `docs/plans/completed/duplicate-fresh-branch.md`.
