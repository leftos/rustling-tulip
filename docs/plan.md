...
- **Protocol breaking changes are free.** Nothing has shipped yet, so subordinate plans that propose `#[serde(default)]` shims, dual-decode paths, or coordinated `PROTOCOL_VERSION` bumps can simplify: just change the wire format. The version bumps as needed (`PROTOCOL_VERSION` is at 8 as of iter 12 — Codex support, preset preview, terminal-title split, RepoStatus index/worktree split, TabContent enum have each rolled it forward); on-disk migration shims still exist where state.json shape changed (`OrphanMeta`, `RepoEntry`, `TabEntry`).
...
- [ ] **Source Control sidebar** — see `docs/plans/source-control-sidebar.md`. VSCode-style activity-bar-driven sidebar with Monaco diff editor, stage/unstage/commit/discard/stash. Phases A (read-only sidebar + path-folded ChangesTree, iters 7–8), B (Monaco diff tabs as their own tab kind, iters 11–12), and C (stage/unstage/commit, iter 9) are **shipped**. Phases D (discard + stash) and E (GraphList pagination + load-more) are still open. *Exit criterion:* all 5 phases (A–E) in the plan tick.
