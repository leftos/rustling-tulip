...
- [ ] **Next up:** Phase 4 (flows and settings). It is a *design* item: split it into brief-sized `P4.<m>` items from its parity sections, and put the split to the user before dispatching. Phases 1–3 are complete. Phases 1 and 2 are complete. See [native-client.md](./plans/native-client.md) for the items and their decided designs.
...
- [x] Native client: move the window-cloaking code (`show_cloaked`, `cloak_and_show`, `OFFSCREEN_ENV` in `apps/native/src/lib.rs`) into a small public module, so `apps/native/examples/diff_bench.rs` calls it instead of carrying a copy
- [ ] Daemon: in `handle_git_write` and `handle_stash_write` (`crates/daemon/src/server.rs`), the branch where the repo path lookup fails after a successful write only warns. A repo unregistered mid-write leaves the writer's pending state stuck. Answer the writer with `GitWriteError` there too, as the refresh-failure branch now does.
- [x] `.\rt.ps1 build` fails in a shell with no `vswhere.exe` on PATH: `Get-VsWherePath` finds it by absolute path, but `VsDevCmd.bat` then calls it by bare name (`rt.ps1` ~300-308). Put its directory on PATH for the `VsDevCmd` call
- [ ] The `Shutdown` handler comment in `crates/daemon/src/server.rs` (~2163, "Pre-Phase-C the children die anyway…") predates tracers: sessions now survive a daemon exit and are reattached or listed as abandoned
- [ ] Decide whether to backport to the Tauri app (frozen, but still shipped until cutover) the terminal-link fixes the native client got in P2.3: Tauri runs a Ctrl+clicked executable or script without asking and touches any UNC host a link or session folder names; and P2.6's copy-output fix: Tauri's gutter-dot menu reads output from the row after the C mark, so with real shells it drops the first output line and copies the next prompt
