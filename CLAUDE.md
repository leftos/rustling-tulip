...
- `native-ui.json` — native client UI state that stays on this machine: sidebar width, the sidebar's collapsed flag and the collapsed containers, the active tab, the quick-shell folder, the app-level terminal font and colours, the shared list of recent custom colours, per-tab font sizes, and the source-control panel's pinned repo and collapsed sections (`apps/native/src/sidebar.rs`). Its `unc_hosts` list (network hosts terminal links may open) is edited by hand; the client re-reads it on each click and never writes it. The native client's bundled OFL fonts live in `apps/native/assets/fonts/` (sources and versions in its README), and the large-file hook exempts them.
...
- A client that needs to know which reply answers its request sets the optional `request_id` on `SpawnSession` / `DuplicateSession` / `LoadScrollback` / `SetSessionAppearance`. The daemon echoes it only on the reply it sends to that requester (`SessionUpdated` or `Scrollback`, `CheckoutConfirmRequired` when an in-place spawn needs a checkout confirm, or `Error` / `ActionFailed` on failure). Broadcasts never carry it. An appearance change's echo travels on the requester's ordered session-event stream (`SessionEvent::Updated` carries its origin connection), so it never overtakes an older broadcast. A `Scrollback` with `forwarder_restarted: true` means the old forwarder has stopped and the new one starts after this reply, so output a client held back before it is already in the history.
