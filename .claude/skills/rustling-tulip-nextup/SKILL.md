...
- **GPUI in the workspace makes cold builds heavy.** Once `apps/native` is a member, `cargo clippy --workspace` and `cargo test --workspace` compile GPUI; a fresh worktree's first build does it from scratch. Give concurrent implementers one shared `CARGO_TARGET_DIR` (`../rustling-tulip.wt/.target`): cargo's build lock serializes their builds, but dependencies are compiled only once.
